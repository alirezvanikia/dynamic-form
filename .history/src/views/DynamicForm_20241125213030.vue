    ...
    <div class="max-w-2xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div v-for="field in formConfig.fields" :key="field.name">
                <InputText v-if="field.name === 'text'" :name="field.name" :label="field.label"
                    v-model="formData[field.name]" :placeholder="field.placeholder" :required="field.required" />
                <InputNumber v-if="field.name === 'number'" :name="field.name" :label="field.label"
                    v-model="formData[field.name]" :placeholder="field.placeholder" :required="field.required" />
                <InputCheckbox v-if="field.name === 'checkbox'" :name="field.name" :label="field.label"
