<template>
    <div class="max-w-2xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        <h1 class="text-2xl font-bold text-gray-800">{{ formConfigData.title }}</h1>
        <p class="text-gray-600 mb-4">{{ formConfigData.description }}</p>
        <form @submit.prevent="handleSubmit" class="space-y-4">
            <!-- حلقه برای رندر فیلدها -->
            <div v-for="field in formConfigData.fields" :key="field.name">
                <InputText v-if="field.name === 'name'" :name="field.name" :label="field.label"
...
export default {
    ...
    data() {
        ...
        formConfigData.fields.forEach((field) => {
        ...
        return {
            //formConfigData: formConfig, // داده‌های فرم از JSON
