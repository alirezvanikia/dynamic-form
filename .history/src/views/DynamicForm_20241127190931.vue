<template>
    <div class="form-body max-w-2xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        <h1 class="text-2xl font-bold text-gray-800">{{ formExtent.title }}</h1>
        <p class="text-gray-600 mb-4">{{ formExtent.description }}</p>
        
        <!-- لودر دایره‌ای -->
        <div v-if="isSubmitting" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <button type="submit"
                class="w-full bg-cyan-800 text-white py-2 px-4 rounded-md shadow-sm hover:bg-cyan-800 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" 
